...
// Ported near-verbatim from mentor.html's inline <script type="module">
// (raw Three.js scene setup + rAF animation loop). This is inherently
// imperative code driving a <canvas> — there's nothing to gain from
// "reactifying" it, so it stays as a single effect that builds the
// scene once and exposes {setExpression, setPalette} via onReady.
function buildNova(canvas, THREE) {
...
export default function NovaAvatar({ onReady }) {
  ...
  useEffect(() => {
    ...
    import('three').then((THREE) => {
      ...
      const api = buildNova(canvasRef.current, THREE);
      apiRef.current = api;
