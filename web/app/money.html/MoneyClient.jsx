...
// This page's interactivity (charts, donut, activity log, tab switching,
// edit-in-place rows, deduct-from-net-worth chooser, ticker) is entirely
// self-contained imperative DOM logic with no benefit from reactifying —
// same call as NovaAvatar's Three.js scene. It's ported near-verbatim
// into this effect, driving the real DOM ids rendered below, exactly as
// the original page's inline <script> did after DOMContentLoaded.
function bootMoney() {
  // ---------- daterow (ported from sunpath.js's fillDaterow) ----------
  const drEl = document.getElementById('spDaterow');
  if (drEl) {
    const d = new Date();
    const DAYS = ['SUNDAY', 'MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY'];
    const MONS = ['JANUARY', 'FEBRUARY', 'MARCH', 'APRIL', 'MAY', 'JUNE', 'JULY', 'AUGUST', 'SEPTEMBER', 'OCTOBER', 'NOVEMBER', 'DECEMBER'];
    drEl.innerHTML = '<span>MONEY</span><span>·</span><span>' + DAYS[d.getDay()] + ', ' + MONS[d.getMonth()] + ' ' + d.getDate() + '</span>';
  }
  ...
  let exchangeRates = { SGD: 1, USD: 1, EUR: 1, GBP: 1 };
  ...
  function fmtMoney(amountCHF) {
    const symbol = currencyEl ? currencyEl.value : 'SGD';
  ...
  function renderActivity() {
    ...
    list.innerHTML = arr.slice(0, 30).map(e => {
      const meta = NW_SLICE_META[e.cat] || { name: e.cat, color: '#F4F1EA' };
  ...
  function renderAllocationDonut(catTotals, grand) {
    ...
    const slices = [];
    NW_CATS.forEach(cat => {
      const meta = NW_SLICE_META[cat.key] || { color: '#F4F1EA' };
      const items = storeGet('nw:' + cat.key) || [];
      items.forEach((it, i) => {
  ...
  function renderSubs() {
    ...
    items.forEach((it, idx) => {
      ...
      const origHint = (it.entered_currency && it.entered_currency !== 'SGD' && it.entered_amount != null)
  ...
  function editSubInline(idx) {
    ...
    const row = rows[idx];
    if (!row) return;
    const enteredCcy = it.entered_currency || 'SGD';
    ...
    const ccyOpts = ['SGD','USD','EUR','GBP'].map(c =>
  ...
  function doSubAdd() {
    try {
      ...
      const enteredCcy = ccEl ? ccEl.value : 'SGD';
  ...
  function renderWishlist() {
    ...
    items.slice().sort((a, b) => (b.amount || 0) - (a.amount || 0)).forEach((it) => {
      ...
      row.innerHTML =
        ...
        +   '<div class="wish-row-meta">' + (it.entered_currency || 'SGD') + ' ' + Number(it.entered_amount != null ? it.entered_amount : it.amount).toLocaleString('en-US', {maximumFractionDigits: 2}) + ' · added ' + fmtActivityDate(it.ts || Date.now()) + '</div>'
        + '</div>'
  ...
  function doWishAdd() {
    try {
      ...
      const aRaw = parseFloat(aEl.value);
      if (!n || isNaN(aRaw)) { nEl.focus(); return; }
      const enteredCcy = cEl ? cEl.value : 'SGD';
      const rate = exchangeRates[enteredCcy] || 1;
      const amountCHF = aRaw / rate;
      const arr = storeGet('wishlist') || [];
      arr.push({
  ...
  function updateOrdPreview() {
    ...
    const ccy = ccyEl ? ccyEl.value : 'SGD';
  ...
  window.__moneyRefreshAll = function () {
    // Call renderAllNetWorth() directly rather than relying on renderSubs()
    // to cascade into it — renderSubs() returns early when there are no
    // subscriptions, before it reaches its own trailing renderAllNetWorth()
    // call, which would otherwise silently skip refreshing net worth for
    // anyone with an empty subs list.
    try { renderAllNetWorth(); renderSubs(); safeRenderTicker(); } catch (e) { console.error(e); }
  };
}

export default function MoneyClient() {
  // Must run BEFORE useCloudSync's effect below installs its
  // localStorage.setItem monkeypatch — renderAllNetWorth() writes a
  // first-paint nw:history snapshot from (still-empty) local data, and if
  // that write happened through an already-installed sync patch it would
  // schedule a push that overwrites real cloud data with a zero snapshot
  // before the cloud pull has a chance to land. React fires effects in
  // hook-call order on mount, so declaring this effect first reproduces
  // the safety the original static page got for free from script order
  // (its inline render script ran before sync.js's deferred script).
  useEffect(() => {
    bootMoney();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  usePull({ goals: { keys: ['goal_streak_v1'] } });
  useCloudSync({
    appKey: 'finance',
    syncedKeys: ['subs', 'wishlist', 'incoming_orders', 'nw_currency', 'nw:activity', 'nw:history'],
    syncedPrefixes: ['nw:'],
    onApplied: () => window.__moneyRefreshAll && window.__moneyRefreshAll(),
  });

  return (
    <div className="money-page">
      <div className="shellwrap">
        <div className="daterow" id="spDaterow"></div>

        <div className="pagehead">
          <h1 id="spHead">
            Money<em>.</em>
          </h1>
          <div className="sub" id="spSub"></div>
        </div>

        <div className="section fin-subnav-wrap">
          <nav className="fin-subnav glassy" id="finSubnav" aria-label="Finance sections">
            <span className="fin-subnav-lens" id="finSubnavLens"></span>
            <button className="fin-subnav-btn active" data-tab="net" type="button">
              <span className="fin-subnav-icon">📊</span>
              <span className="fin-subnav-label">Net Worth</span>
            </button>
            <button className="fin-subnav-btn" data-tab="subs" type="button">
              <span className="fin-subnav-icon">🔁</span>
              <span className="fin-subnav-label">Subs</span>
            </button>
            <button className="fin-subnav-btn" data-tab="incoming" type="button">
              <span className="fin-subnav-icon">📦</span>
              <span className="fin-subnav-label">Orders</span>
            </button>
            <button className="fin-subnav-btn" data-tab="wish" type="button">
              <span className="fin-subnav-icon">🎯</span>
              <span className="fin-subnav-label">Wishlist</span>
            </button>
          </nav>
        </div>

        {/* ===== FINANCE TICKER (rotating subscription renewals) ===== */}
        <div className="finance-ticker hidden" id="financeTicker">
          <span className="ticker-tag" id="tickerTag">RENEWS</span>
          <div className="ticker-stream" id="tickerStream"></div>
          <div className="ticker-dots" id="tickerDots"></div>
        </div>

        {/* ===== NET WORTH ===== */}
        <div className="section" data-section="net">
          <div className="section-title">
            <span className="section-title-text">NET WORTH</span>
          </div>
          <div className="card" style={{ background: 'transparent', border: 'none', boxShadow: 'none', padding: 0, overflow: 'visible' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 500 }}>Total net worth</div>
                <div id="netWorthTotal" style={{ fontSize: 34, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.05, marginTop: 2 }}>SGD 0</div>
                <div id="netWorthBreakdown" style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 4 }}></div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>SGD</span>
                <select id="netWorthCurrency" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 8px', fontSize: 12, fontFamily: 'inherit', cursor: 'pointer' }}>
            ...
            <div className="nw-overview-grid">
              <div className="nw-chart-wrap" id="nwChartWrap">
                <div className="nw-chart-head">
                  <span className="nw-chart-label">All-time</span>
                  <span className="nw-chart-delta" id="nwChartDelta">—</span>
                </div>
                <svg className="nw-chart-svg" id="nwChartSvg" viewBox="0 0 600 200" preserveAspectRatio="none" aria-hidden="true">
                  <defs>
                  ...
                  <line className="nw-chart-grid" x1="0" x2="600" y1="40" y2="40" />
                  <line className="nw-chart-grid" x1="0" x2="600" y1="100" y2="100" />
                  <line className="nw-chart-grid" x1="0" x2="600" y1="160" y2="160" />
                ...
                <div className="nw-chart-stats" id="nwChartStats">
                  <div className="nw-stat">
                    <div className="nw-stat-label">1% =</div>
                    <div className="nw-stat-val" id="nwStat1pct">—</div>
                  </div>
                  <div className="nw-stat">
                    <div className="nw-stat-label">All-time high</div>
                    <div className="nw-stat-val" id="nwStatHigh">—</div>
                  </div>
                  <div className="nw-stat">
                    <div className="nw-stat-label">All-time low</div>
                    <div className="nw-stat-val" id="nwStatLow">—</div>
                  </div>
                  <div className="nw-stat">
                    <div className="nw-stat-label">Snapshots</div>
                    <div className="nw-stat-val" id="nwStatCount">—</div>
                  </div>
                </div>
                <div className="nw-chart-empty" id="nwChartEmpty">Add or edit an asset to start tracking your net worth over time.</div>
              </div>

              <div className="nw-donut-wrap">
                <div className="nw-chart-head">
                  <span className="nw-chart-label">Allocation</span>
                  <span className="nw-chart-delta" id="nwDonutCount">—</span>
                </div>
                <div className="nw-donut-stage">
                  <svg className="nw-donut-svg" id="nwDonutSvg" viewBox="0 0 140 140" aria-hidden="true"></svg>
                  <div className="nw-donut-center">
                    <div className="nw-donut-num" id="nwDonutTotal">—</div>
                    <div className="nw-donut-sub">total</div>
                  </div>
                </div>
                <div className="nw-donut-empty" id="nwDonutEmpty">Add an account to see your breakdown</div>
                <div className="nw-donut-legend" id="nwDonutLegend"></div>
              </div>
            </div>

            {/* Activity log — every add / edit, newest first */}
            <div className="nw-activity" id="nwActivity">
              <div className="nw-activity-h">
                <span className="nw-chart-label">Recent activity</span>
                <span className="nw-chart-delta" id="nwActivityCount">—</span>
              </div>
              <div className="nw-activity-list" id="nwActivityList"></div>
              <div className="nw-activity-empty" id="nwActivityEmpty">No activity yet — add your first account.</div>
            </div>

            <div className="card-grid" style={{ gap: 10 }}>
              {/* Bank accounts */}
              <div className="card nw-subcard">
                <div className="label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span>🏦 Bank accounts</span>
                  <span id="bankTotal" style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 600 }}>0</span>
                </div>
                <div id="bankList" className="nw-list"></div>
                <div className="quick-add">
                  <input type="text" placeholder="Account name" id="bankName" />
                  <input type="number" placeholder="Amount" id="bankAmount" step="0.01" />
                  <button className="quick-add-btn" id="bankAddBtn">+</button>
                </div>
              </div>
              {/* Stocks / Investments */}
              <div className="card nw-subcard">
                <div className="label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span id="stocksTotal" style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 600 }}>0</span>
                </div>
                <div id="stocksList" className="nw-list"></div>
                <div className="quick-add">
                  <input type="text" placeholder="Stock / fund" id="stocksName" />
                  <input type="number" placeholder="Value" id="stocksAmount" step="0.01" />
                  <button className="quick-add-btn" id="stocksAddBtn">+</button>
                </div>
              </div>
              {/* Crypto */}
              <div className="card nw-subcard">
                <div className="label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span>🪙 Crypto</span>
                  <span id="cryptoTotal" style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 600 }}>0</span>
                </div>
                <div id="cryptoList" className="nw-list"></div>
                <div className="quick-add">
                  <input type="text" placeholder="Coin / wallet" id="cryptoName" />
                  <input type="number" placeholder="Value" id="cryptoAmount" step="0.01" />
                  <button className="quick-add-btn" id="cryptoAddBtn">+</button>
                </div>
              </div>
              {/* Other assets */}
              <div className="card nw-subcard">
                <div className="label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span id="otherTotal" style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 600 }}>0</span>
                </div>
                <div id="otherList" className="nw-list"></div>
                <div className="quick-add">
                  <input type="text" placeholder="Asset name" id="otherName" />
                  <input type="number" placeholder="Value" id="otherAmount" step="0.01" />
                  <button className="quick-add-btn" id="otherAddBtn">+</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="section" data-section="subs">
          <div className="section-title">
            <span className="section-title-text">ACTIVE SUBSCRIPTIONS</span>
          </div>
          <div className="card" style={{ background: 'linear-gradient(135deg, rgba(83,74,183,0.07), rgba(216,90,48,0.05))', border: '1px solid rgba(83,74,183,0.20)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12, marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 500 }}>Monthly burn</div>
                <div id="subsTotal" style={{ fontSize: 28, fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1.1, marginTop: 2 }}>
                  SGD 0 <span style={{ fontSize: 13, color: 'var(--text-tertiary)', fontWeight: 500 }}>/ mo</span>
                </div>
                <div id="subsYearly" style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 2 }}></div>
              </div>
              <div id="subsCount" style={{ fontSize: 11, color: 'var(--text-tertiary)' }}></div>
            </div>
            <div id="subsList"></div>
            <div className="empty-state" id="subsEmpty">No subscriptions yet</div>
            <div className="quick-add" style={{ flexWrap: 'wrap', gap: 6 }}>
              <input type="text" placeholder="Service (e.g. Netflix)" id="subName" style={{ flex: 1.5, minWidth: 120 }} />
              <input type="number" placeholder="Amount" id="subAmount" step="0.01" style={{ flex: 1, maxWidth: 100 }} />
              <select id="subCurrency" style={{ padding: '6px 10px', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-md)', fontSize: 12, fontFamily: 'inherit', background: 'var(--bg-card)', color: 'var(--text-primary)', maxWidth: 90 }}>
                <option value="SGD">SGD</option>
                <option value="USD">USD</option>
                <option value="EUR">EUR</option>
                <option value="GBP">GBP</option>
              </select>
              <select id="subPeriod" style={{ padding: '6px 10px', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-md)', fontSize: 12, fontFamily: 'inherit', background: 'var(--bg-card)', color: 'var(--text-primary)', maxWidth: 100 }}>
                <option value="monthly">Monthly</option>
                <option value="yearly">Yearly</option>
                <option value="weekly">Weekly</option>
              </select>
              <label className="date-field" title="Next renewal date" style={{ minWidth: 150 }}>
                <span className="date-emoji" aria-hidden="true">📅</span>
                <input type="date" id="subRenewal" />
              </label>
              <select id="subFromCat" title="Which net worth account to deduct from" style={{ padding: '6px 10px', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-md)', fontSize: 12, fontFamily: 'inherit', background: 'var(--bg-card)', color: 'var(--text-primary)', minWidth: 130 }}>
                <option value="">No accounts yet</option>
              </select>
              <label className="sub-auto-toggle" title="Auto-deduct from the linked account on each renewal">
                <input type="checkbox" id="subAutoDeduct" />
                <span className="sub-auto-track">
                  <span className="sub-auto-thumb"></span>
                </span>
                <span className="sub-auto-label">Auto-deduct</span>
              </label>
              <button className="quick-add-btn" id="subAddBtn" onClick={() => window.__addSub && window.__addSub()}>+ Add</button>
            </div>
          </div>
        </div>

        {/* ===== INCOMING ORDERS ===== */}
        <div className="section" data-section="incoming">
          <div className="section-title">
            <span className="section-title-text">INCOMING ORDERS</span>
          </div>

          {/* Add card with cost + currency + from-category + arrival date */}
          <div className="card ord-add-card">
            <div className="ord-add-eyebrow">+ NEW ORDER</div>
            <div className="ord-add-grid">
              <input type="text" id="ordName" placeholder="Item (e.g. New iPhone)" className="ord-input ord-input-name" />
              <input type="number" id="ordCost" placeholder="Cost" step="0.01" className="ord-input ord-input-cost" />
              <select id="ordCurrency" className="ord-input ord-input-ccy">
                <option value="SGD">SGD</option>
                <option value="USD">USD</option>
                <option value="EUR">EUR</option>
                <option value="GBP">GBP</option>
              </select>
              <select id="ordFromCat" className="ord-input ord-input-from" title="Which net worth account this comes out of">
                <option value="">No accounts yet</option>
              </select>
              <label className="date-field ord-input-date" title="Expected arrival">
                <span className="date-emoji" aria-hidden="true">📅</span>
                <input type="date" id="ordArrival" />
              </label>
              <button className="ord-add-btn" id="ordAddBtn" onClick={() => window.__addOrder && window.__addOrder()}>+ Add Order</button>
            </div>
            <div className="ord-add-preview" id="ordAddPreview">Type a cost — preview will show what % of net worth it takes.</div>
          </div>

          {/* Items list */}
          <div className="ord-section-h">
            <span className="ord-section-label">ITEMS ON THE WAY</span>
            <span className="ord-section-count" id="ordCount">0 items</span>
          </div>
          <div id="ordList"></div>
          <div className="ord-empty-card" id="ordEmpty">
            <div className="ord-empty-icon">📦</div>
            <div className="ord-empty-title">Nothing on the way</div>
            <div className="ord-empty-sub">Log an order above. Pick the account it comes out of, the expected arrival date, and the dashboard will show what % of your net worth it costs.</div>
          </div>
        </div>

        {/* ===== WISHLIST ===== */}
        <div className="section" data-section="wish">
          <div className="section-title">
            <span className="section-title-text">WISHLIST</span>
          </div>
          {/* Hero card with stats */}
          <div className="card wish-hero">
            <div className="wish-hero-top">
              <div>
                <div className="wish-hero-eyebrow">WISHLIST TOTAL</div>
                <div className="wish-hero-num" id="wishTotal">SGD 0</div>
              </div>
              <div className="wish-hero-pct-wrap">
                <div className="wish-hero-pct-label">% OF NET WORTH</div>
                <div className="wish-hero-pct-num" id="wishPctOfNwHero">—</div>
              </div>
            </div>
            <div className="wish-hero-bar">
              <div className="wish-hero-bar-fill" id="wishHeroFill" style={{ width: '0%' }}></div>
            </div>
            <div className="wish-hero-foot" id="wishPctOfNw">Add net worth first to see this as a %</div>
          </div>

          {/* Add row */}
          <div className="card wish-add-card">
            <div className="wish-add-eyebrow">+ ADD AN ITEM</div>
            <div className="wish-add-row">
              <input type="text" id="wishName" placeholder="Item name (e.g. New iPhone Pro Max)" className="wish-input wish-input-name" />
              <input type="number" id="wishAmount" placeholder="Cost" step="0.01" className="wish-input wish-input-amt" />
              <select id="wishCurrency" className="wish-input wish-input-ccy">
                <option value="SGD">SGD</option>
                <option value="USD">USD</option>
                <option value="EUR">EUR</option>
                <option value="GBP">GBP</option>
              </select>
              <button className="wish-add-btn" id="wishAddBtn" onClick={() => window.__addWish && window.__addWish()}>+ Add</button>
            </div>
          </div>

          {/* Items list */}
          <div className="wish-section-h">
            <span className="wish-section-label">YOUR WISHES</span>
            <span className="wish-section-count" id="wishCount">0 items</span>
          </div>
          <div id="wishList"></div>
          <div className="wish-empty-card" id="wishEmpty">
            <div className="wish-empty-icon">🎯</div>
            <div className="wish-empty-title">No wishes yet</div>
            <div className="wish-empty-sub">Add anything you're saving for — the dashboard will calculate what % of your net worth it'd cost.</div>
          </div>
        </div>
      </div>
      <AvatarBubble />
